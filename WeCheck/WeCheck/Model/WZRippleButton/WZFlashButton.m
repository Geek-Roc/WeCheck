...
//                                                   radius:radius];
    ...
    for (int i = 0; i < 3; i++) {
        CAAnimationGroup *groupAnimation = [self createFlashAnimationWithScale:scale*4 duration:3];
        /* Use KVC to remove layer to avoid memory leak */
        [groupAnimation setValue:circleShape forKey:@"circleShaperLayer"];
        groupAnimation.autoreverses = NO;
        groupAnimation.repeatCount = HUGE_VAL;//HUGE_VALF;     //HUGE_VALF,源自math.h
        
        groupAnimation.beginTime = CACurrentMediaTime()+i*3/3;
